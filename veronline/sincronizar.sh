...
sshpass -p $senha_f rsync -vazhP -e "ssh -p $porta_f" $user_f@veronline.rocks:/VM/hosts /etc/hosts

echo "Sincronizar Certificados"
sshpass -p $senha_f rsync -vazhP --partial --inplace -e "ssh -p $porta_f" $user_f@$master:/VM/veronline/certificado/ /home/ubuntu/certificado/
...
sshpass -p $senha_f rsync -vazhP --partial --inplace --delete --exclude '*.txt' --exclude 'midia' --exclude 'stats' -e "ssh -p $porta_f" $user_f@$master:/VM/$base/www/ /var/www/html/
...
#echo "testeveronli" > /var/www/html/stats/idrouter.dat
