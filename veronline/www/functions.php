...
function getEspacoLivre() {
  $stuff =exec("du -sh /var/www/html");
  $stuff = str_replace('/var/www/html', '', $stuff);
  return '<div align="right"><h3>Espaço utilizado: '.trim($stuff).'B (de 64GB)</h3></div>';
}
...
function getCategoriasSelf($id_vm){
$filmesS = array();
$filmesTxt = fopen("filmes.txt", "r") or die("Arquivo de filmes não encontrado! (sinopse)");
$filmes = explode('|', fread($filmesTxt,filesize("filmes.txt")));
 fclose($filmesTxt);
...
foreach ($filmes as $f) {  
      $fs = explode('$', $f);
      if (trim($fs[0]) == trim($id_vm)) return $fs[1];
    }
